...
public class EAElementDAO extends EADao<EAElementVO> {
	...
	public EAElementVO getEAElement(String uri) {
		EAElementVO v= findVO("select ea_guid,Package_id,Object_id,Name,Alias,Note,Stereotype,object_type,pdata1 from t_object where ea_guid=?",new Object[]{uri});
		//Root models are not stored in t_object table - try lookup in t_package table
		if (v==null) {
			v= findVO("select ea_guid,parent_id,package_id,name,name,notes,null,'Package' as type,package_id from t_package t where ea_guid=?",new Object[]{uri});
		}
		return v;
	}
	...
	public Collection<EAElementVO> getPackageElements(String uri) {
		return findVOs("select ea_guid,Package_id,Object_id,Name,Alias,Note,Stereotype,object_type,pdata1 from t_object where package_id=(select pdata1 from t_object where ea_guid=?)",new Object[]{uri});
	}
